...
export class InitDB1732711144847 implements MigrationInterface {
    name = 'InitDB1732711144847'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`category\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`category_name\` varchar(255) NOT NULL, \`color_badge\` enum ('default', 'primary', 'secondary', 'error', 'info', 'success', 'warning') NOT NULL DEFAULT 'primary', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
